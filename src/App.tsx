...
export default function App() {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape') { event.preventDefault(); setResetPending(false); return }
  ...
  return <div className="shell">
    ...
    <main>
      ...
      {view === 'command' && <>
        ...
        <section className="scorebar" aria-label="Launch readiness summary"><div className="score"><strong>{status.passed}<small> / {status.total}</small></strong><span>Required gates ready</span></div><div className="progress" role="progressbar" aria-valuemin={0} aria-valuemax={status.total} aria-valuenow={status.passed} aria-label="Required gates ready"><span style={{ width: `${status.total ? status.passed / status.total * 100 : 0}%` }} /></div><div className="score-note" aria-live="polite">{status.blockers.length ? <><AlertTriangle size={17} /><b>{status.blockers.length} blocker{status.blockers.length === 1 ? '' : 's'} before launch</b></> : <><CircleCheck size={17} /><b>All required gates cleared</b></>}</div></section>
        ...
        <aside className="decision-panel">
          ...
          <label className="rationale"><span>Decision rationale</span><textarea disabled={!prelaunchOpen} maxLength={MAX_RATIONALE_LENGTH} aria-label="Decision rationale" value={decisionNote} onChange={(event) => setDecisionNote(event.target.value)} placeholder="What supports this decision?" rows={3} /></label>
    ...
    {resetPending && <div className="warning" role="dialog" aria-modal="true" aria-labelledby="reset-title" aria-describedby="reset-description"><section ref={resetDialogRef}><strong id="reset-title">Reset this sample?</strong><span id="reset-description">This clears the local launch session and restores the seeded blockers.</span><button type="button" ref={cancelResetRef} onClick={() => setResetPending(false)}>Keep current session</button><button type="button" onClick={reset}>Reset session</button></section></div>}
    <footer><span>Northstar sample · No real systems connected</span><button type="button" ref={resetButtonRef} onClick={() => setResetPending(true)}><RotateCcw size={15} />Reset sample</button><button type="button" onClick={exportSummary}><Download size={15} />Export summary</button><a href="#timeline">View timeline <ChevronRight size={15} /></a></footer>
  </div>
}
